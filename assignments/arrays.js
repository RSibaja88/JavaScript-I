...
for (let b = 0; b < inventory.length; b++) {
    carModels.push(inventory[b]['car_model'])
}
carModels.sort();
...
for (let c = 0; c < inventory.length; c++) {
    carYears.push(inventory[c]['car_year']);
}
...
for (let d = 0; d < inventory.length; d++) {
    let carMake = inventory[d]['car_make']
    if (carMake === "BMW" || carMake === "Audi") {
        BMWAndAudi.push(inventory[d]);
    }
}
